...
def permission(request,*args,**kwargs):
   if 'Permission' in kwargs.get('module') and kwargs.get('access'):
      if request.method == 'POST':
            ...
            listData = []
            if search :
                  data = Role.objects.using('default').filter(name__contains=search,id__in=[1,2])[startIndex:endIndex].all()
                  totalLen = list(Role.objects.using('default').filter(name__contains=search,id__in=[1,2]).all())
            else:
                  data = Role.objects.using('default').filter(id__in=[1,2])[startIndex:endIndex].all()
                  totalLen = list(Role.objects.using('default').filter(id__in=[1,2]).all())
